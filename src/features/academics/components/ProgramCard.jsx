...
const ProgramCard = ({ program, onExplore }) => {
  ...
  const tagThemes = {
    ...
    bbm: "text-purple-700 bg-purple-50 border-purple-100",
    bsw: "text-rose-700 bg-rose-50 border-rose-100",
  };

  const programKey = String(program.code ?? "").toLowerCase();
  const displayCode = String(program.code ?? "").toUpperCase();
  const currentBadgeColor = badgeColors[programKey] || "bg-stone-600";
  const currentTagTheme = tagThemes[programKey] || "text-stone-700 bg-stone-50 border-stone-100";
    ...
    <div className="bg-white border border-stone-200 rounded-3xl overflow-hidden shadow-xs flex flex-col justify-between hover:shadow-md transition-shadow duration-300 text-left h-full">
      ...
      <div className="relative h-48 sm:h-52 w-full bg-stone-100 overflow-hidden">
        ...
        <img 
          loading="lazy" 
          decoding="async"
          // Logic: If image exists, show it; otherwise, use placeholder
          src={program.image || "/placeholder.png"}
          alt={program.title}
          className="w-full h-full object-cover"
        />

        <div className={`absolute top-4 left-4 z-20 ${currentBadgeColor} text-white font-bold text-xs px-3 py-1 rounded-full uppercase tracking-wider`}>
          {displayCode}
        </div>
